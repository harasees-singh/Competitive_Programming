...
int32_t main(){
    ...
    if(n > m){
        ...
        // consider the prefix sum array of ai's. atleast 2 of these have the same modulo m since n > m; 
        // which implies u can consider dat subarray and it's sum will be zero modulo m.
        ...
        // dp[i] = 1 : means a rem of i is possible till now.
        ...
        loop(i, 0, n){
            ...
            loop(j, 0, m){
                ...
                if(dp[j]) mujhe_one_karo.pb((curr + j)%m);
            }

            dp[curr] = 1;

            for(auto p : mujhe_one_karo) dp[p] = 1;
