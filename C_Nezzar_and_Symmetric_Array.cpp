...
int32_t main(){
    ...
    test_cases_loop{
        ...
        loop(i, 0, 2*n){
            ...
            input.insert(t); if(t%2)parity = false;

            frequency_map[t]++;

            if(frequency_map[t] == 3)parity = false;
        }

        if(sz(input) != n) parity = false;

        if(*(--input.end())%(2*n)) parity = false;

        // if(!parity)continue;

        int prev = *(--input.end())/(2*n);
        ...
        while(curr != input.begin()){

            double ele = prev - (*curr - *(--curr))/(2.0*count);

            // if(abs(diff)%(count))parity = false;

            if(ele <= 0 or (int)ele != ele){parity = false; break;}

            // int ele = diff + prev;

            count--;

            // if(ele < 0){ parity = false; break;}
