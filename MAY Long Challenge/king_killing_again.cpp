...
set<int> Graph::DFS(int v, int depth, int parent)
{
    ...
    for (int i=0; i<adj[v].size(); i++)
    {
        if (!visited[adj[v][i]] && adj[v][i]!=parent){
            set<int> returned_set = DFS(adj[v][i], curr_depth, v);
    ...
    test_cases_loop
    {
        ...
        for(int i=0; i<g.adj[1].size(); i++){
            ...
            while(set_of_leaves.size()-1){
                ...
                for(int leaf:set_of_leaves){
                    // if(check_for_stay_parity[child_parent[leaf]]){
                    ...
                    if(connection_map[child_parent[leaf]] >= 2){
                        if(!popped[child_parent[leaf]]){
                            ...
                            // check_for_stay_parity[child_parent[leaf]]=true;
                        }
                ...
                for(int leaf:copy_set){
                    if(!popped[child_parent[leaf]] && !check_for_stay_parity[child_parent[leaf]]){
                        set_of_leaves.erase(leaf); set_of_leaves.insert(child_parent[leaf]);
                    }
                    // else if(check_for_stay_parity[child_parent[leaf]]){
                    //     // check_for_stay_parity[child_parent[leaf]]=false;
                    //     continue;
                    // }
                    else{
        ...
        cout << answer_set.size() << " " << answer_number_of_days << '\n';
