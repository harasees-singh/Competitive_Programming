...
int32_t main(){
    ...
    test_cases_loop{
        ...
        mii intervals_cut_total_cuts;
        int curr = 1;
        int last_index = (*(prefix_sum.begin())).first;
        for(auto p : prefix_sum){

            intervals_cut_total_cuts[curr] = max(p.first - last_index, 0);
            // cout << curr << endl;
            curr += p.second;
        ...
        auto it = --intervals_cut_total_cuts.end();

        int ans = n;

        while(c and it != intervals_cut_total_cuts.begin()){

            ans += min(c, (*it).second) * (*it).first  ;
        ...
        if(c and it == intervals_cut_total_cuts.begin()){
