...
int32_t main(){
    ...
    test_cases_loop{
        ...
        if(n%2){
            ...
            loop(i, 0, n - 1){ans = 2*ans; ans%=MOD;}

            int copy = ans;

            ans = 1;

            loop(i, 0, k){ans *= copy; ans%=MOD;}
            ...
            loop(i, 0, n) num *= 2, num %= MOD, denom *= 2, denom %= MOD;

            int copy = num;

            num = 1;

            loop(i, 0, k) num *= copy, num%=MOD;

            num--; num = (num+MOD)%MOD; denom--; denom = (denom+MOD)%MOD;

            if(k==0) num=0;

            cout << (num*modInverse(denom, MOD) + 1)%MOD << endl;
        }
    }
