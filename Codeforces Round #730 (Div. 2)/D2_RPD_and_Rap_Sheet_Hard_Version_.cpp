...
int xora(int a, int b, int k){

    vi first = k_bit(a, k), second =  k_bit(b, k);

    first.resize(max(sz(first), sz(second)));

    second.resize(max(sz(first), sz(second)));

    int ans = 0;

    loop(i, 0, sz(first)){

        ans += (power_modulus(k, i))*((first[i] + second[i])%k);
    }
...
int32_t main(){
    ...
    test_cases_loop{
        ...
        loop(i, 0, n){
            ...
            int p = xora(complement(k_bit(last, k), k), i, k);
    ...
    // for(auto a:k_bit(9, 3))cout << a << space;
