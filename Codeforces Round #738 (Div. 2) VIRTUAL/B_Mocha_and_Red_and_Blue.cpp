...
int32_t main(){
    ...
    test_cases_loop{
        ...
        while(find < n and s[find] == '?') find++;

        if(find == n) s = 'R' + s, find = 0;

        int pairty = (find%2) ^ (s[find] == 'B'); loop(i, 0, find){pairty ? out.pb('B') : out.pb('R'); pairty ^= 1;}
        ...
        while(find != n){
            ...
            if(s[find] == 'B') parity = 0;

            // parity ? out.pb('R') : out.pb('B');

            out.pb(s[find]);

            find++;

            while(find < n and s[find] == '?'){
                
                char answer; parity ? answer = 'B' : answer = 'R'; out.pb(answer); 
