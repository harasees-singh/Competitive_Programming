...
int32_t main(){
    ...
    int moda = abs(a), modb = abs(b);

    vi numbers;

    int left = 0, right = modb, mid;
    ...
    while(left <= right){
        ...
        if(count_digits(mid)*n > 10){right = mid - 1; continue;}

        int val = moda*pow_good(mid, n);

        if(val == modb) {found = true; break;}

        if(val > modb) right = mid - 1;
    ...
    if(found){
        ...
        if(a < 0 ^ b < 0){
            ...
            if(n%2 or !mid) cout << -mid << endl; else cout << "No solution" << endl;
        }
