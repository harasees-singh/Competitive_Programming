...
int32_t main(){
    ...
    loop(i, 2, sqrt(n) + 1){
        ...
        if(!(n%i)){
            ...
            prime_factors.pb(i); n/=i;

            while(!(n%i)) prime_factors[sz(prime_factors) - 1] *= i, n/=i;
        }
    }

    if(n - 1) prime_factors.pb(n);

    int minima = INT64_MAX;

    pii ans;

    loop(i, 0, (1 << sz(prime_factors))){

        int a = 1, b = 1;

        loop(j, 0, sz(prime_factors)){

            if((i >> j)&1) a *= prime_factors[j];
            
            else b *= prime_factors[j];
        }

        // minima = min(minima, max(a, b));

        if(minima > max(a, b)) ans = {a, b}, minima = max(a, b);
    }
