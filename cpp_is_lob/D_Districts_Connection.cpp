...
int32_t main(){
    ...
    test_cases_loop{
        ...
        loop(i, 0, n) {cin >> input[i]; prohibited += em[input[i]]; em[input[i]]++;}

        if(((n)*(n-1))/2 - prohibited < n - 1) cout << "NO" << endl;
            ...
            for(int i = 0; i < n; i++) if(input[i] != connection) {cout << 1 << space << i + 1 << endl; mark = i;}

            for(int i = 1; i < n; i++) if(input[i] == connection) cout << i + 1 << space << mark + 1 << endl;
        }
    }
