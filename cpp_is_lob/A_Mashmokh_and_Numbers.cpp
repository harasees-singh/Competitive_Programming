...
int32_t main(){
    ...
    if(k==0 and n==1){ cout << 7 << endl; exit(0);}

    if(k < n/2 or n==1) {cout << -1 << endl; exit(0);}

    int temptations = k - n/2 + 1;

    cout << temptations << space << temptations*2 << space;

    int print = temptations*2;

    loop(i, 0, n-2) cout << ++print << space;
