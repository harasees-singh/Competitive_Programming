...
int dfs(int curr, int n, vi &input, vi &dp){
    ...
    while(it*curr <= n){
        ...
        if(input[curr*it] <= input[curr]){it++; continue;}

        int temp;

        if(dp[curr*it]) temp = 1 + dp[curr*it];

        else temp = 1 + dfs(curr*it, n, input, dp);
