...
int32_t main(){
    ...
    unordered_map<string , string> usernameOLDNEW;
    ...
    loop(i, 0, n){
        ...
        string old, latest; cin >> old >> latest;


        if(store.count(old) == 0) used.insert(old);

        store.insert(old); store.insert(latest);
        usernameOLDNEW[old] = latest;
    }
    ...
    for(auto p : used){
        ...
        while(usernameOLDNEW[old] != "") old = usernameOLDNEW[old];
