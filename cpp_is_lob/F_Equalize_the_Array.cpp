...
int32_t main(){
    ...
    test_cases_loop{
        ...
        unordered_map<int, int, custom_hash> ele_frequency;

        loop(i, 0, n){int t; cin >> t; ele_frequency[t]++;}

        vi sortkaromujhe;

        for(auto &p: ele_frequency){

            int count = p.second;

            sortkaromujhe.pb(count);
        }

        sort(all(sortkaromujhe));

        // print<int>(sortkaromujhe);

        int dynamic = 0;

        loop(t, 1, sortkaromujhe[sz(sortkaromujhe) - 1] + 1){

            int discard = lower_bound(all(sortkaromujhe), t) - sortkaromujhe.begin();

            // cout << "discard " << discard << endl;

            dynamic = max(dynamic, (sz(sortkaromujhe) - discard)*t);
        }
