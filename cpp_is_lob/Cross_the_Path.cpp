...
int32_t main(){
    ...
    test_cases_loop{
        string s; cin >> s;
        int start = 1e4 * (2e4 + 1) + 1e4;
        bool parity= true;
        unordered_set<int> ellie_golding; ellie_golding.insert(start);
        for(auto a: s){
            ...
            if(ellie_golding.find(start) != ellie_golding.end()){parity = false; cout << 1 << endl; break;} 

            ellie_golding.insert(start);
        }
        if(parity) cout << 0 << endl;
    }
    return 0;
}
