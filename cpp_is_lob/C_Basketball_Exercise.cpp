...
int32_t main(){
    ...
    loop(i, 0, n){int t; cin >> t; upar.pb(t);}

    loop(i, 0, n){int t; cin >> t; neeche.pb(t);}

    int leave = 0, up__ = 0, down__ = 0;

    loop(i, 0, n){
        
        int t1, t2, t3;

        t1 = max(leave, max(up__, down__));

        t2 = max(upar[i] + leave, upar[i] + down__);

        t3 = max(neeche[i] + leave, neeche[i] + up__);

        leave = t1, up__ = t2, down__ = t3;
    }

    cout << max(leave, max(up__, down__)) << endl;
