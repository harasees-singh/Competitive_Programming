...
int32_t main(){
    ...
    vi trees; loop(i, 0, n){int t; cin >> t; trees.pb(t);}
    ...
    loop(i, 0, n){
        ...
        if(trees[i] > trees[tree_fix_it] + k*(i - tree_fix_it)){

            cout << '-' << space << i + 1 << space << trees[i] - (trees[tree_fix_it] + k*(i - tree_fix_it)) << endl;
        }

        else if(trees[i] < trees[tree_fix_it] + k*(i - tree_fix_it)){
