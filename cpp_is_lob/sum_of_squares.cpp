...
int dfs(int state){
    ...
    int sqroot = *lower_bound(all(jcole_numbers), round(sqrt(state)));

    if(sqroot*sqroot == state){
    ...
    loop(i, 1, (int)sqrt(5*10000000)){
        
        if(i*i > state) break;

        int state_change = state - i*i;

        if(dp.count(state_change)){

            ans = min(ans, dp[state_change] + 1);
        }
        
        else{

            dp[state_change] = dfs(state_change);
