...
int32_t main(){
    ...
    vi deadlyConnections(n+1);

    loop(i, 1, n+1){

        for(auto a : adjacencyList[i]){

            deadlyConnections[i] += (bool)(a > i);
        }

        dead += (bool)(deadlyConnections[i]);
    }
    ...
    loop(i, 0, q){
        ...
        if(parity == 1){
            ...
            // add friendship :: kill people

            int a, b; cin >> a >> b;

            deadlyConnections[min(a, b)]++;

            if(deadlyConnections[min(a, b)] == 1){alive--;}
        }
        ...
        if(parity == 2){
            ...
            if(deadlyConnections[min(a, b)] == 0){alive++;}
        }
    }
